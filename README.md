...
1. 你在服务器配置里填写 `CDT 每月重置日`，默认是 `1`。
2. 服务器因为达到停机阈值被自动关机后，面板会计算下一次账期重置日期。
3. 到达重置日后，定时巡检会再次查询 CDT 流量。
4. 如果流量已经低于恢复启动阈值，并且这台机器不是手动关机保持状态，面板会自动开机。
...
5. 回到面板点击 `获取 Chat ID`。
6. 面板会列出候选 Chat ID，点击 `使用这个 Chat ID`。
